...
struct PlyFile::PlyFileImpl {
  ...
  void write_property_ascii(Type t, std::ostream &os, uint8_t *src,
                            size_t &srcOffset);
  void write_property_binary(std::ostream &os, uint8_t *src, size_t &srcOffset,
...
void PlyFile::PlyFileImpl::write_property_binary(
    std::ostream &os, uint8_t *src, size_t &srcOffset,
    const size_t &stride) noexcept {
      ...
      for (auto &p : e.properties) {
        ...
        if (p.isList) {
          ...
          write_property_binary(
              os, (helper->data->buffer.get() + helper->cursor->byteOffset),
